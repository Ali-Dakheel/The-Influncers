...
- **Never edit existing migrations.** Always `php artisan make:migration` for changes. (Enforced by hook in repo root `.claude/settings.json`.)
...
- **Never run `git commit` or `git push`.** Hook blocks them; user commits manually.
- **Never read `.env` or `.env.*`.** Hook blocks them; structure documented in `PROJECT_CONTEXT.md`.
